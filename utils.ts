...
export const COLORS: string[] = ['#7c5cff', '#ec4899', '#f59e0b', '#60a5fa', '#10b981', '#f43f5e'];
export const LIKE_EMOJIS: string[] = ['✨','🎉','💖','🥳','💯','🤪','😍','🙌','🥰','🤩','👌'];

export const openInNewTab = (url: string): void => { 
  window.open(url, '_blank', 'noopener'); 
};

export const setAriaInvalid = (el: HTMLElement | null, isInvalid: boolean): void => {
  if (!el) return;
...
export const createConfettiParticle = (x: number, y: number, dx: number, dy: number): HTMLDivElement => {
...
export const createConfettiEmoji = (x: number, y: number, dx: number, dy: number, emojis: string[]): HTMLDivElement => {
