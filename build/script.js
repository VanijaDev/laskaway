...
function generateCardStack(experiences) {
...
function generateCarousel(experiences) {
    return experiences.map(exp => {
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const createConfettiParticle = (x, y, dx, dy) => {
        ...
        if (shape !== 'triangle' && shape !== 'ribbon') {
            ...
            if (Math.random() > 0.6)
                piece.style.width = piece.style.height = '10px';
        }
        return piece;
    };
    ...
    emailInput?.addEventListener('input', () => {
        ...
        if (typeof refreshJoinHoverState === 'function')
            refreshJoinHoverState();
    });
    form?.addEventListener('submit', async (e) => {
        e.preventDefault();
        if (!emailInput || !joinBtn || !waitlistJoinSuccess || !form)
            return;
    ...
    function refreshJoinHoverState() {
        if (!joinBtn)
            return;
        if (joinBtn.classList.contains('submitting'))
            return; // keep disabled during submit
        if (emailInput?.disabled)
            return; // skip when input is disabled during submit
    ...
    if (stack) {
        ...
        const updateStackBases = (randomizeBottom = false) => {
            ...
            const children = Array.from(stack.children);
            if (children[0])
                setRandomBase(children[0]); // randomize new bottom after a swipe
        };
